...
from textblob.classifiers import DecisionTreeClassifier, NaiveBayesClassifier, MaxEntClassifier, NLTKClassifier
...
class MyClassifier(NLTKClassifier):
    nltk_class = nltk.classify.SklearnClassifier
...
def search_department(job, train):
    cl_depart = MyClassifier(train)
    prob_dist = cl_depart.classify(job)
    print(prob_dist)
    return prob_dist
